...
```{r eval=FALSE}
d1<-d%>%dplyr::select(-c(menhIndx:RaoQ, ss_dst_timelag))%>%
  na.omit()
```
...
Richness_m11<-lme4::glmer(richness ~ 1 + (1|cas_id/survey_year) + (1|SS), data=d,  family = poisson, na.action = na.exclude)

Richness_m12<-lme4::glmer(richness ~ 1 + (1|SS/survey_year) + (1|cas_id), data=d,  family = poisson, na.action = na.exclude)

Richness_m13<-lme4::glmer(richness ~ 1 + (1|cas_id/survey_year), data=d,  family = poisson, na.action = na.exclude)
# Compare models
...
anova(Richness_m13, Richness_m11)


Richness_null_ms <- model.sel(Richness_m13, rank="AIC", extra = c(r2=function(x) round(r.squaredGLMM(x)[1,c(1,2)],3)))

save(Richness_null_ms, file="2_pipeline/store/models/richness_null_ms.rData")

Richness_null<-Richness_m13
```
...
# RdNBR
nmp1<-lme4::glmer(richness ~RdNBR + (1|cas_id/survey_year), data=d,  family = poisson, na.action = na.exclude, control=glmerControl(optCtrl=list(maxfun=1e9))) 

nmp2<-lme4::glmer(richness ~poly(RdNBR,2) + (1|cas_id/survey_year), data=d, family = poisson, na.action = na.exclude, control=glmerControl(optCtrl=list(maxfun=1e9))) 

nmp3<-lme4::glmer(richness ~poly(RdNBR,3) + (1|cas_id/survey_year), data=d,  family = poisson, na.action = na.exclude, control=glmerControl(optCtrl=list(maxfun=1e9))) 

nmp4<-lme4::glmer(richness ~poly(RdNBR,4) + (1|cas_id/survey_year), data=d,  family = poisson, na.action = na.exclude, control=glmerControl(optCtrl=list(maxfun=1e9))) 
...
pred<-effect_plot(nmp2, pred=RdNBR, data = d1,  plot.points=TRUE, interval=TRUE)
plot(pred)
...
nmp5<-lme4::glmer(richness ~dNBR_recovery + (1|cas_id/survey_year), data=d,  family = poisson, na.action = na.exclude, control=glmerControl(optCtrl=list(maxfun=1e9))) 

nmp6<-lme4::glmer(richness ~poly(dNBR_recovery,2) + (1|cas_id/survey_year), data=d,  family = poisson, na.action = na.exclude, control=glmerControl(optCtrl=list(maxfun=1e9))) 

nmp7<-lme4::glmer(richness ~poly(dNBR_recovery,3) + (1|cas_id/survey_year), data=d,  family = poisson, na.action = na.exclude, control=glmerControl(optCtrl=list(maxfun=1e9))) 

nmp8<-lme4::glmer(richness ~poly(dNBR_recovery,4) + (1|cas_id/survey_year), data=d,  family = poisson, na.action = na.exclude, control=glmerControl(optCtrl=list(maxfun=1e9))) 
...
nmp9<-lme4::glmer(richness ~ss_dst_timelag_nbr + (1|cas_id/survey_year), data=d,  family = poisson, na.action = na.exclude, control=glmerControl(optCtrl=list(maxfun=1e9))) 

nmp10<-lme4::glmer(richness ~poly(ss_dst_timelag_nbr,2) + (1|cas_id/survey_year), data=d,  family = poisson, na.action = na.exclude, control=glmerControl(optCtrl=list(maxfun=1e9))) 

nmp11<-lme4::glmer(richness ~poly(ss_dst_timelag_nbr,3) + (1|cas_id/survey_year), data=d,  family = poisson, na.action = na.exclude, control=glmerControl(optCtrl=list(maxfun=1e9))) 

nmp12<-lme4::glmer(richness ~poly(ss_dst_timelag_nbr,4) + (1|cas_id/survey_year), data=d,  family = poisson, na.action = na.exclude, control=glmerControl(optCtrl=list(maxfun=1e9))) 
...
```{r eval=FALSE}
nbr_1<-glmer(richness ~ RdNBR+ dNBR + dNBR_perc+ (1|cas_id/survey_year), data=d,  family = poisson,  na.action=na.fail) 
...
```{r}
in_1<-glmer(richness ~ poly(ss_dst_timelag_nbr,2, raw=TRUE) * dNBR_perc + (1|cas_id/survey_year), data=d,  family = poisson,  na.action=na.fail) 
d_in_1<-MuMIn::dredge(in_1, extra = c(r2=function(x) round(r.squaredGLMM(x)[1,c(1,2)],3)))


in_2<-lme4::glmer(richness ~ poly(dNBR_recovery,2) * poly(RdNBR,2)   +  (1|cas_id/survey_year), data=d,  family = poisson,  na.action=na.fail) 
...
gm1<-lme4::glmer.nb(richness ~  poly(ss_dst_timelag_nbr,2, raw=TRUE) * poly(dNBR_perc ,2, raw=TRUE) + Shape_Area + PA_ratio +  ss_dst_timelag_nbr +dist_to_edge + dNBR_recovery + NBR_survey + Coniferous +Exposed_Barren_land + Herbs + Mixedwood + Shrubs +  Water + Wetland + Wetland_treed +
canopy_height + canopy_standard_deviation +
lat + (1|cas_id/survey_year), data=d,na.action = na.fail, control=glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e9))) 


# gm1a<-lme4::glmer.nb(richness ~  ss_dst_timelag_nbr + dNBR_perc + Shape_Area + PA_ratio +  ss_dst_timelag_nbr +dist_to_edge + dNBR_recovery + NBR_survey + Coniferous +Exposed_Barren_land + Herbs + Mixedwood + Shrubs +  Water + Wetland + Wetland_treed +
# canopy_height + canopy_standard_deviation +
# lat + (1|cas_id/survey_year), data=d,  na.action = na.fail, control = glmerControl(optimizer = "bobyqa"))

summary(gm1)
#Check variance inflation factors of fixed effects
l_vif<-as.data.frame(vif(gm1))
l_vif

richness_dredge<-MuMIn::dredge(gm1, extra = c(r2=function(x) round(r.squaredGLMM(x)[1,c(1,2)],3)))
...
m1<-lme4::glmer(richness ~  poly(dNBR_perc,2, raw=TRUE)* poly(ss_dst_timelag, 2 ,raw=TRUE)  + dist_to_edge + (1|cas_id/survey_year), data=d2,  family = poisson, na.action = na.fail, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e9))) 

m2<-lme4::glmer(richness ~ poly(ss_dst_timelag_nbr,2, raw=TRUE) * poly(RdNBR,2, raw=TRUE) + dist_to_edge + (1|cas_id/survey_year), offset=log(n_samples), data=d,family = poisson, na.action = na.fail) 

m2a<-lme4::glmer(richness ~ poly(ss_dst_timelag_nbr,2, raw=TRUE) * poly(dNBR_perc,2, raw=TRUE) + (1|cas_id/survey_year), data=d,family = poisson, na.action = na.fail, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e9))) 

m3<-lme4::glmer(richness ~ ss_dst_timelag * poly(RdNBR,2) + (1|cas_id/survey_year), offset=log(n_samples), data=d,family = poisson, na.action = na.fail, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e9))) 

m4<-lme4::glmer(richness ~ poly(ss_dst_timelag,2) * RdNBR + (1|cas_id/survey_year), offset=log(n_samples), data=d,family = poisson, na.action = na.fail, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e9))) 
...
m2<-lme4::glmer(richness ~ poly(ss_dst_timelag,2) * poly(RdNBR,2) + dist_to_edge + (1|cas_id/survey_year), data=d, family = poisson, na.action = na.fail, control=glmerControl(optCtrl=list(maxfun=1e9))) 
...
m1<-lme4::glmer(richness ~  poly(dNBR_perc,2, raw=TRUE)* poly(ss_dst_timelag_nbr, 3 ,raw=TRUE)  + dist_to_edge + (1|cas_id/survey_year), data=d1,  family = poisson, na.action = na.fail, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e9))) 


m2<-lme4::glmer(richness ~ poly(dNBR_perc,2, raw=TRUE)+ poly(ss_dst_timelag_nbr, 3 ,raw=TRUE)  + dist_to_edge + (1|cas_id/survey_year), data=d1,  family = poisson, na.action = na.fail, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e9))) 
...
m3<-lme4::glmer(richness ~  poly(dist_ext_upper_1_sc,2, raw=TRUE) + poly(ss_dst_timelag_nbr, 3 ,raw=TRUE)  + dist_to_edge + (1|cas_id/survey_year), data=d1,  family = poisson, na.action = na.fail, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e9))) 
