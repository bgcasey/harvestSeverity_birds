...
richness_m1<-lme4::glmer(richness ~  poly(ss_dst_timelag_nbr,2, raw=TRUE) * poly(dNBR_perc ,2, raw=TRUE) + Coniferous +Exposed_Barren_land + Herbs + Shrubs +  Water + canopy_standard_deviation +
...
richness_m1_noOffset<-lme4::glmer(richness ~  poly(ss_dst_timelag_nbr,2, raw=TRUE) * poly(dNBR_perc ,2, raw=TRUE) + Coniferous +Exposed_Barren_land + Herbs + Shrubs +  Water + canopy_standard_deviation +
...
sc_1 <- attr(test1$ss_dst_timelag_nbr, 'scaled:scale')
ce_1 <- attr(test1$ss_dst_timelag_nbr, 'scaled:center')

sc_2 <- attr(test1$dNBR_perc, 'scaled:scale')
ce_2 <- attr(test1$dNBR_perc, 'scaled:center')
# make predictions from model using ggpredict
library(ggeffects)
dd2<-ggpredict(richness_m1_noOffset2, terms = c("ss_dst_timelag_nbr [all]","dNBR_perc [all]"))%>%#, #condition = c(sampling_intensity = 1440))%>%
         as_tibble()%>%
...
shan_m1<-lme4::glmer(shan ~  poly(ss_dst_timelag_nbr,2, raw=TRUE) * poly(dNBR_perc ,2, raw=TRUE) + Coniferous +Exposed_Barren_land  +  Water  + canopy_standard_deviation + Mixedwood +
lat + (1|HFI_ID/survey_year)+ offset(log(sampling_intensity)), data=d, family = Gamma(link = log),  na.action = na.fail, control=glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e9)))
...
shan_m1_noOffset<-lme4::glmer(shan ~  poly(ss_dst_timelag_nbr,2, raw=TRUE) * poly(dNBR_perc ,2, raw=TRUE) + Coniferous +Exposed_Barren_land  +  Water  + canopy_standard_deviation + Mixedwood +
...
dd2<-ggpredict(shan_m1, terms = c("ss_dst_timelag_nbr [all]","dNBR_perc [all]"))%>%#, #condition = c(sampling_intensity = 1440))%>%
         as_tibble()%>%
  mutate(aa = as.numeric(as.character(group))* sc_2 + ce_2)%>%
    # mutate(dNBR_perc_us_us = as.numeric(group))%>%
  mutate(timelag = x* sc_1 + ce_1) %>%
...
FRic_m1<-lme4::glmer(FRic ~  poly(ss_dst_timelag_nbr,2, raw=TRUE) * poly(dNBR_perc ,2, raw=TRUE) +  dist_to_edge+ Shrubs +Herbs + Mixedwood  +  Water  +  (1|HFI_ID/survey_year)+ offset(log(sampling_intensity)), data=d, family = Gamma(link = log),  na.action = na.fail, control=glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e9)))

save(FRic_m1, file="3_output/FRic_m1.rData")

FRic_m1_noOffset<-lme4::glmer(FRic ~  poly(ss_dst_timelag_nbr,2, raw=TRUE) * poly(dNBR_perc ,2, raw=TRUE) +  dist_to_edge+ Shrubs +Herbs + Mixedwood  +  Water  +  (1|HFI_ID/survey_year), data=d, family = Gamma(link = log),  na.action = na.fail, control=glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e9)))
...
dd2<-ggpredict(FRic_m1, terms = c("ss_dst_timelag_nbr [all]","dNBR_perc [all]"))%>%#, #condition = c(sampling_intensity = 1440))%>%
         as_tibble()%>%
  mutate(aa = as.numeric(as.character(group))* sc_2 + ce_2)%>%
    # mutate(dNBR_perc_us_us = as.numeric(group))%>%
  mutate(timelag = x* sc_1 + ce_1) %>%
...
FEve_m1<-lme4::glmer(FEve ~  poly(ss_dst_timelag_nbr,2, raw=TRUE) * poly(dNBR_perc ,2, raw=TRUE) + Coniferous + Mixedwood + Shrubs +  Water  + canopy_standard_deviation +
lat + dist_to_edge + (1|HFI_ID/survey_year)+ offset(log(sampling_intensity)), data=d, family = Gamma(link = log),  na.action = na.fail, control=glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e9)))

summary(FEve_m1)

save(FEve_m1, file="3_output/FEve_m1.rData")

FEve_m1_noOffset<-lme4::glmer(FEve ~  poly(ss_dst_timelag_nbr,2, raw=TRUE) * poly(dNBR_perc ,2, raw=TRUE) + Coniferous + Mixedwood + Shrubs +  Water  + canopy_standard_deviation +
lat + dist_to_edge + (1|HFI_ID/survey_year), data=d, family = Gamma(link = log),  na.action = na.fail, control=glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e9)))
...
dd2<-ggpredict(FEve_m1, terms = c("ss_dst_timelag_nbr [all]","dNBR_perc [all]"))%>%#, #condition = c(sampling_intensity = 1440))%>%
         as_tibble()%>%
  mutate(aa = as.numeric(as.character(group))* sc_2 + ce_2)%>%
    # mutate(dNBR_perc_us_us = as.numeric(group))%>%
  mutate(timelag = x* sc_1 + ce_1) %>%
...
dd2<-ggpredict(FDiv_m1, terms = c("ss_dst_timelag_nbr [all]","dNBR_perc [all]"))%>%#, #condition = c(sampling_intensity = 1440))%>%
         as_tibble()%>%
  mutate(aa = as.numeric(as.character(group))* sc_2 + ce_2)%>%
    # mutate(dNBR_perc_us_us = as.numeric(group))%>%
  mutate(timelag = x* sc_1 + ce_1) %>%
...
FDis_m1<-lme4::glmer(FDis ~  poly(ss_dst_timelag_nbr,2, raw=TRUE) * poly(dNBR_perc ,2, raw=TRUE) + dist_to_edge + Coniferous +Exposed_Barren_land + Herbs + lat+ Shrubs + Water + canopy_standard_deviation + (1|HFI_ID/survey_year)+ offset(log(sampling_intensity)), data=d, family = Gamma(link = log),  na.action = na.fail, control=glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e9)))

save(FDis_m1, file="3_output/FDis_m1.rData")

FDis_m1_noOffset<-lme4::glmer(FDis ~  poly(ss_dst_timelag_nbr,2, raw=TRUE) * poly(dNBR_perc ,2, raw=TRUE) + dist_to_edge + Coniferous +Exposed_Barren_land + Herbs + lat+ Shrubs + Water + canopy_standard_deviation + (1|HFI_ID/survey_year), data=d, family = Gamma(link = log),  na.action = na.fail, control=glmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=1e9)))

save(FDis_m1_noOffset, file="3_output/FDis_m1.rData_noOffset")
...
dd2<-ggpredict(FDis_m1, terms = c("ss_dst_timelag_nbr [all]","dNBR_perc [all]"))%>%#, #condition = c(sampling_intensity = 1440))%>%
         as_tibble()%>%
  mutate(aa = as.numeric(as.character(group))* sc_2 + ce_2)%>%
    # mutate(dNBR_perc_us_us = as.numeric(group))%>%
  mutate(timelag = x* sc_1 + ce_1) %>%
