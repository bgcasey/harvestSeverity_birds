...
valid<-as.data.frame(st_is_valid(abmi, reason = TRUE))
nonvalid<-valid%>%filter(is.na(valid))
...
#check
valid_2<-as.data.frame(st_is_valid(abmi_2, reason = TRUE))
nonvalid_2<-valid_2%>%filter(is.na(valid_2))

## create a 30 m negative buffer
# check units
st_crs(abmi_2)$units
#buffer
abmi_buff<-st_buffer(abmi_2, dist=-30)
# check geometries
valid_3<-as.data.frame(st_is_valid(abmi_buff, reason = TRUE))
nonvalid_3<-valid_3%>%filter(is.na(valid_3))

as.data.frame(table(st_geometry_type(abmi_buff)))
abmi_buff = st_cast(abmi_buff, "MULTIPOLYGON")

# simplify
abmi_buff_s<-st_simplify(abmi_buff, dTolerance = 5)

plot(abmi_buff[2,]$Shape)
plot(abmi_buff_s[2,]$Shape)
...
Calculate distance of pointcoints to nearest harvest edge.
...
ss_xy_harvest<-st_join(ss_xy, casfri_harvest)%>%
  ...
  dplyr::mutate(dist_to_edge=st_distance(ss_xy, HFI_ID))
...
#   filter(!is.na(HFI_ID))
# nrow(as.data.frame(unique(test$HFI_ID)))
# #3679           
```
