...
for (i in 1:length(origFiles)) {
  ...
  fileName <- substr(fullName, nchar(paste0(filepath, "unprocessed/")), nchar(fullName))
...
fullData <- rfmtDat[order(rfmtDat$OBJECTID),]
str(fullData)

save(fullData, file="0_data/manual/gee_tables/nbr/fulDataNBR.rData")
```

```{r fulDataNBR_in_table, echo=FALSE, results="asis"}
load("2_pipeline/store/fulDataNBR.rData")
...
save(finalFullTable, file="0_data/manual/gee_tables/nbr/finalFullTable.rData")
#--- Write to csv
fwrite(finalFullTable, file=paste0(filepath,"NBR_allMets_finalFullTable.csv"))
...
poly<-st_read("0_data/manual/spatial/harvest/o18_HarvestAreas_HFI_2019_buff_neg30_surveyed_simp.shp")

# finalFullTable2<-finalFullTable%>%
#   left_join(poly)

# %>%
#   dplyr::select(-c(geometry, Shap_Ar, Shp_Lng, SOURCE, FEATURE, SECTOR_, Mdfr_Yr))
...
t<-anti_join(cleanData_na, cleanData,  c("OBJECTID", "HFI_ID", "YEAR")
dim(t) #2730

t$highConf="n"
cleanData$highConf="y"

nbr_cleanData<-rbind(cleanData,t)

save(nbr_cleanData_in, file="2_pipeline/store/nbr_cleanData.rDATA")
#--------------------------------------------------------------------------------------------
