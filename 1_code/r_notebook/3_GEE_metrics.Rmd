...
gee_metrics_2<-gee_metrics%>%  
  ...
  dplyr::filter(landcover_yr==srvy_yr)%>%
...
save(gee_metrics_3, file="2_pipeline/tmp/gee_metrics_3.rData")
```
...
origFiles <- list.files(path=paste0(filepath, 'raw'), pattern="*.csv$", full.names=TRUE, recursive=FALSE)
```
...
#--- For each file, do...
for (i in 1:length(origFiles)) {
  
  #--- Message to user
  print(paste("Reformatting file", i, "of", length(origFiles), "..."))
  
  #--- Import data from current file
  thisDat <- fread(origFiles[i], data.table = FALSE)
  ...
  flagData <- data.frame("TOT_PIXELS"=0,"PERC_UNMSKD"=0,"UNMSKD_PIXELS"=0,"MAX_CONNCTDPIX"=0,"PERC_NOHARVEST"=0,
                         "PERC_MULTIDISTB"=0,"PERC_OUTOFRANGE"=0,"PERC_NOREGEN"=0,"PERC_NO5REGEN"=0,
                         "PERC_NO10REGEN"=0,"PERC_NO15REGEN"=0,"PERC_NO20REGEN"=0)
  
  #--- List of relevant columns
  flagCols <- c("TOT_PIXELS","PERC_UNMSKD","UNMSKD_PIXELS","MAX_CONNCTDPIX","PERC_NOHARVEST","PERC_MULTIDISTB",
                "PERC_OUTOFRANGE","PERC_NOREGEN","PERC_NO5REGEN","PERC_NO10REGEN",
                "PERC_NO15REGEN","PERC_NO20REGEN")
  ...
  #outMetTable_fin <- outMetTable_fin[, c(1, 46:48, 2:45)]
  ...
  fullName <- substr(origFiles[i], 1, nchar(origFiles[i])-4)
  fileName <- substr(fullName, nchar(filepath)+4, nchar(fullName))
  extension <- substr(origFiles[i], nchar(origFiles[i])-3, nchar(origFiles[i]))
  outputName <- paste0(filepath,'reformatted',fileName,"_RFMTTD",extension)
  #outputName <- paste0('../LandsatNBR/output/tables/reformatted/',fileName,"_RFMTTD",extension)
  
  
  #--- Write to file
  fwrite(outMetTable_fin, file = outputName)
...
##### Compile reformatted files into one {-}


```{r compile_reformatted_files, eval=FALSE}

#--- Get list of names for new reformatted metric files
rfmtFiles <- list.files(path=paste0(filepath, 'reformatted'), pattern="*_RFMTTD.csv", full.names=TRUE, recursive=TRUE)

#--- Read in first file to create data frame
rfmtDat <- fread(rfmtFiles[1], data.table = FALSE)

#--- For remaining files, read in data and add to existing data frame
for (m in 2:length(rfmtFiles)) {
thisFile <- fread(rfmtFiles[m], data.table = FALSE)
 rfmtDat <- rbind(rfmtDat, thisFile)
}

#--- Create new data frame for further additions, and modifications
fullData <- rfmtDat[order(rfmtDat$OBJECTID),] #--- n = 172606 obs. of 45 variables

str(fullData)

save(fullData, file="2_pipeline/store/fulDataNBR_in.rData")
```

```{r fulDataNBR_in_table, echo=FALSE, results="asis"}
load("2_pipeline/store/fulDataNBR_in.rData")
...
##### Calculate confidence scores and add to data frame {-}
...
save(finalFullTable, file="2_pipeline/store/finalFullTable_in.rData")
#--- Write to csv
fwrite(finalFullTable, file=paste0(filepath,"/final/NBR_allMets_finalFullTable.csv"))

```


```{r echo=FALSE, results="asis"}
load("2_pipeline/store/finalFullTable_in.rData")

knitr::kable(head(finalFullTable)) %>%
  kable_styling(font_size = 10, bootstrap_options= c("striped", "hover", "condensed" ), full_width= F, position="center") %>%
  column_spec(1, width= "20em")%>%
  scroll_box(width = "100%", height = "400px")
```
...
##### Remove inappropriate, low-confidence, and/or irrelevant records {-}
...
cleanData <- cleanData[which(cleanData$UNMSKD_PIXELS >= 9),]
dim(cleanData) #--- n = 4733 obs.
...
cleanData <- cleanData[which(cleanData$MAX_CONNCTDPIX >= 11),]
dim(cleanData)

#--- Remove rows/data records wherein >50% of pixels are flagged and masked out
length(which(cleanData$PERC_UNMSKD < 50)) #--- n = 15792
cleanData <- cleanData[which(cleanData$PERC_UNMSKD >= 50),]
dim(cleanData) #--- n = 1274 obs.
...
meanConfScr <- mean(cleanData$sumConf) #mean = 39.26253
meanConfScr
sdConfScr <- sd(cleanData$sumConf)   #sd = 5.054748
sdConfScr

sd2below <- meanConfScr - (2 * sdConfScr)
sd2below #2 stdev below mean = 29.15304
sd3below <- meanConfScr - (3 * sdConfScr)
sd3below #3 stdev below mean = 24.09829

dim(cleanData[which(cleanData$sumConf <= 24 |
...
cleanData <- cleanData[which(cleanData$sumConf >= 24),]
dim(cleanData) #n = 1160
...
t<-anti_join(cleanData_na, cleanData, by = c("OBJECTID", "SS", "cas_id", "srvy_yr"))
dim(t) #7191

t$highConf="n"
cleanData$highConf="y"

nbr_cleanData_in<-rbind(cleanData,t)

save(nbr_cleanData_in, file="2_pipeline/store/nbr_cleanData_in.rDATA")
#--------------------------------------------------------------------------------------------
#--- Output clean data set to CSV
#--------------------------------------------------------------------------------------------

#--- Write to csv
fwrite(nbr_cleanData_in, file=paste0(filepath,"final/nbr_cleanData_in.csv"))

```


The final table has a column that categorizes data by data I'm confident in and data that I'm not confident in.

```{r showNBRtable, echo=FALSE, results="asis"}
load("2_pipeline/store/nbr_cleanData_in.rDATA")
knitr::kable(head(nbr_cleanData_in)) %>%
  kable_styling(font_size = 12, bootstrap_options= c("striped", "hover", "condensed" ), full_width= F, position="center") %>%
  column_spec(1, width= "20em")%>%
  scroll_box(width = "100%", height = "400px")
```
